...
//             GetComponent<Rigidbody>().AddForce(data.direction * speed, ForceMode.VelocityChange);
//         }
...
public class Player : NetworkBehaviour
{
    ...
    private void Update()
    {
        if (cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + rotationCamera.x * mouseSensitivity;
        
            if (!invertCamera)
            {
                pitch -= mouseSensitivity * rotationCamera.y;
            }
            else
            {
                // Inverted Y
    ...
    public override void FixedUpdateNetwork()
     {
         ...
         {    
             if (GetInput(out NetworkInputData data))
             {
                 rotationCamera = data.rotation;
                 
                 yaw = transform.localEulerAngles.y + data.rotation.x * mouseSensitivity;
             }
         }
         ...
         if (playerCanMove)
         {
             ...
             {
                 CheckGround();
                 if (enableJump && isGrounded && data.isJump)
                 {
                     Jump();
                 }
                 ...
                 Vector3 targetVelocity = data.direction;
